# resampling allows to change the rate at which data is taken
...
TX_1yr = HPI_data['TX'].resample('A').mean()  # resample the data annually using mean of all the data of the year
print(TX_1yr.head())


fig = plt.figure()
ax1 = plt.subplot2grid((1, 1), (0, 0))  # creates a 1 by 1 grid starting at (0,0)
HPI_data['TX'].plot(ax=ax1, label='monthly')  # can plot an entire data like this dataframe.plot()
TX_1yr.plot(ax=ax1, label='yearly')
